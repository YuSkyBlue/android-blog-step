    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.buttonConnect.setOnClickListener {            //클라이언트   ->   서버   접속
            ...
            if (binding.etIp.text.isNotEmpty()) {
                ...
                if (binding.etPort.text.isNotEmpty()) {
                    ...
                    Toast.makeText(this@MainActivity, "PORT   번호를   입력해주세요.", Toast.LENGTH_SHORT)
                        .show()
                }
            } else {
        ...
        binding.buttonDisconnect.setOnClickListener {            //클라이언트   ->   서버   접속   끊기
            if (!socket.isClosed) {
                ...
                Toast.makeText(this@MainActivity, "서버와   연결이   되어있지   않습니다.", Toast.LENGTH_SHORT)
                    .show()
            }
        }
        ...
        mHandler = object : Handler(Looper.getMainLooper()) {      //Thread들로부터   Handler를   통해   메시지를   수신
                override fun handleMessage(msg: Message) {
                    ...
                    when (msg.what) {
                        ...
                        13 -> Toast.makeText(
                            this@MainActivity,
                            (msg.obj as String) + "   클라이언트와   연결되었습니다.",
                            Toast.LENGTH_SHORT
                        ).show()
                        ...
                        16 -> Toast.makeText(
                            this@MainActivity,
                            (msg.obj as String) + "   클라이이언트와의   연결을   종료합니다.",
                            Toast.LENGTH_SHORT
                        ).show()
